...
(function() {
  ...
  const isWrapperLink = (element) => {
    ...
    const blockishTags = new Set([
      ...
      'p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6',
      'ul', 'ol', 'li', 'figure', 'figcaption', 'table', 'thead', 'tbody', 'tr', 'td', 'th',
      'form'
    ]);
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      if (blockishTags.has(childTag)) return true;
  ...
  const isWrapperButton = (element) => {
    ...
    // If it wraps layout/content blocks, treat it as a section-like wrapper, not a simple button.
    for (let i = 0; i < children.length; i++) {
      const child = children[i];
      const childTag = child.tagName ? child.tagName.toLowerCase() : '';
      if (blockishTags.has(childTag)) return true;
    ...
    // If it contains multiple nested blocks, it's almost certainly a wrapper.
    if (element.querySelector('div, section, article, main, header, footer, nav, aside, ul, ol, li, table, form')) {
      return true;
    }
  ...
  if (backgroundImage && backgroundImage !== 'none') {
    ...
    if (tagName === 'button') return isWrapperButton(element) ? 'Section' : 'Button';
    if (tagName === 'a' && element.getAttribute('href')) return isWrapperLink(element) ? 'Section' : 'Button';
  ...
  const getMostSpecificElement = (x, y) => {
    ...
    // Prefer direct media nodes over their interactive wrappers (e.g. img inside button/link).
    // 'elementsFromPoint' returns elements from topmost to bottommost, so we pick the first media hit.
    const directMedia = validElements.find(function(el) {
      const tag = el && el.tagName ? el.tagName.toLowerCase() : '';
      return tag === 'img' || tag === 'video';
    });
    if (directMedia) return directMedia;

    // If media nodes are not in the hit-test stack (e.g. pointer-events: none),
    // try to resolve an img/video descendant that visually contains the point.
    const findDescendantMediaAtPoint = function(root, px, py) {
    ...
    for (let i = 0; i < validElements.length; i++) {
      ...
      const descendantMedia = findDescendantMediaAtPoint(el, x, y);
      if (descendantMedia && isValidElement(descendantMedia)) return descendantMedia;
    }
    const scoredElements = validElements.map(element => {
      ...
      const isRealLink = element.tagName === 'A' && element.getAttribute('href') && !isWrapperLink(element);
      const isInteractive =
        (element.tagName === 'BUTTON' && !isWrapperButton(element)) ||
        isRealLink ||
      ...
      if (element.tagName === 'A' && isWrapperLink(element)) score -= 20;
